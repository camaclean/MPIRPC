...
#ifndef MPIRPC__INTERNAL__APPLY_BUFFER_HPP
#define MPIRPC__INTERNAL__APPLY_BUFFER_HPP

#include "function_attributes.hpp"

#include "detail/apply_buffer.hpp"
...
void apply(F&& f, Allocator&& a, InBuffer&& s, OutBuffer&& os, bool get_return = false, std::tuple<Alignments...> = std::tuple<Alignments...>{})
{
    ...
    using alignments = custom_alignments<typename mpirpc::internal::function_parts<Func>::default_alignments,std::tuple<Alignments...>>;
    constexpr std::size_t num_args = mpirpc::internal::function_parts<Func>::num_args;
    detail::apply_impl(std::forward<F>(f),std::forward<Allocator>(a),std::forward<InBuffer>(s), std::forward<OutBuffer>(os), get_return, fargs{}, ts{}, pass_backs{}, std::make_index_sequence<num_args>{}, alignments{});
}
...
void apply(std::function<F>& f, Allocator&& a, InBuffer&& s, OutBuffer&& os, bool get_return = false, std::tuple<Alignments...> = std::tuple<Alignments...>{})
{
    ...
    using alignments = custom_alignments<typename mpirpc::internal::function_parts<std::add_pointer_t<Func>>::default_alignments,std::tuple<Alignments...>>;
    constexpr std::size_t num_args = mpirpc::internal::function_parts<std::add_pointer_t<Func>>::num_args;
    detail::apply_impl(f,std::forward<Allocator>(a),std::forward<InBuffer>(s), std::forward<OutBuffer>(os), get_return, fargs{}, ts{}, pass_backs{}, std::make_index_sequence<num_args>{}, alignments{});
}
...
#endif /* MPIRPC__INTERNAL__APPLY_BUFFER_HPP */
